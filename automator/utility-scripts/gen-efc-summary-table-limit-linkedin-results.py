...
def main():
    ...
    OUTPUT_FILE_NAME = "summary-table-limited.out"
    OUTPUT_FILE_VULN_NAME = "summary-table-vulnerability-limited.out"
    ...
    TARGET_URLS_FILE_PATH = os.path.join(AUTOMATOR_DIR, os.path.join(str(siteId), "urls"))
    TARGET_URLS_FILE_PATH_NAME = os.path.join(TARGET_URLS_FILE_PATH, "urls.txt")
    #-------------------------------------------------------------------#
    #                                   End Constants
    #-------------------------------------------------------------------#

    # contains the list of urls to limit the results to
    limit_results_to_urls_list = [] 

    fd = open(TARGET_URLS_FILE_PATH_NAME, "r");
    limit_results_to_urls_list = fd.readlines()
    for i in range(len(limit_results_to_urls_list)):
        limit_results_to_urls_list[i] = "\""+limit_results_to_urls_list[i].strip("\n")+ "\""
    fd.close()
    ...
    with open(TEMP_FILE_PATH_NAME, "r") as tempFP:
        ...
        for eachURL in distinctURLs:
                if eachURL not in limit_results_to_urls_list:
                    continue
                for i in range(len(tempFileLines)):
                        line=tempFileLines[i]
                        if eachURL in line:
    ...
    with open(OUTPUT_FILE_PATH_VULN_NAME, "wb") as outFileFp:
        outFileFp.write("======================================================================\n")
        outFileFp.write("[Subject]: Summary of Vulnerable Results For EventFireCount Attack\n")
        outFileFp.write("[Generated]: %s\n"%timestamp )
        outFileFp.write("======================================================================\n\n\n")
        ...
        for eachKeyURL in resultsDict:
                ...
                for eachCategory in SortedCats:
                        ...
                        for vector in vectors:
                                ...
                                currentCategory = vector[4].split(", ")[0].strip().strip("\"") #added strip("\"")  to fix github qutoation error creating multiple same categories
                        ...
                        for event in diff_cats:
                                ...
                                outFileFp.write("\t\tEvents: {0}\t\tStates: ".format(event))
                                for idx in range(len(vectors)):
                                        vector=vectors[idx]
                                        stateLabel=vector[3].split(", ")[1].strip().split("_")[0].strip() #the last part removes the _EventsFired tag
