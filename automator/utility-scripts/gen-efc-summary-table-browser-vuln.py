...
def main():
    if len(sys.argv)!= 3:
            print "Script-Usage: you must provide siteId and browser name as argument, e.g.\n'python gen-summary-csv.py siteId browsername'"
            return 0

    siteId = sys.argv[1]
    bname= sys.argv[2] #browser name
    ...
    ROOT_DIR = os.path.join(AUTOMATOR_DIR, os.path.join("%s"%siteId, os.path.join("TestReports", os.path.join("EventFireCount","%s"%bname))))
    OUTPUT_DIR = ROOT_DIR
    ...
    OUTPUT_FILE_VULN_NAME = "summary-table-vulnerability-%s.out"%bname
