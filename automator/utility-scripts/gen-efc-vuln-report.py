...
def main():
	...
	ROOT_DIR = os.path.join(AUTOMATOR_DIR, "%s/TestReports/EventFireCount/"%siteId)
	from datetime import datetime
	...
	for subdir, dirs, files in os.walk(ROOT_DIR):
		...
		if len(list_files) != 0:
			...
			with open(subdir+ "/" + VULNERABILITY_REPORT_OUT_NAME, "wb") as outFileFp:
				...
				outFileFp.write("\tThis report is produced at the completion of the EventFireCount test\n\t for the websiteId=%s, and provides a summary of all potentially vulnerable URLS of this website to EventFireCount attack\n"%siteId )
				...
				for name in ListOfDistinctFileNames:
					...
					for header in ListOfDistinctHeaders:
						...
						stateCount = len(states)/2 #each two header value relate to one state in eventfirecount
