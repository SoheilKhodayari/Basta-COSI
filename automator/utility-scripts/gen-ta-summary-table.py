...
def main():
	...
	ROOT_DIR = os.path.join(AUTOMATOR_DIR, os.path.join("%s"%siteId, os.path.join("TestReports", "TimingAnalysis")))
	OUTPUT_DIR = ROOT_DIR
			...
			for i in range(len(tempFileLines)):
				...
				if eachURL in line:
					...
					for j in range(i, 0, -1):
						...
						currentTag = lineHeadingSplitted[2].strip("\n").strip()
	...
	with open(OUTPUT_FILE_PATH_NAME, "wb") as outFileFp:
		...
		outFileFp.write("[Report-TimeUnit]: miliseconds (ms)\n")
		outFileFp.write("======================================================================\n\n\n")
